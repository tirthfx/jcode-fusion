...
| 2 | **Worktree-per-subagent swarm isolation** under jcode's existing coordinator | Codex + Grok Build (convergent) | **Shipped (creation only).** New `crates/jcode-app-core/src/swarm_worktree.rs`, wired into `spawn_swarm_agent` right after working-dir resolution — opt-in (`JCODE_FUSION_SWARM_WORKTREES=1`), only in the "would otherwise share the parent's dir" fallback path, fails open on error. `VersionedPlan` confirmed already durably persisted (`server/swarm_persistence.rs`), not ephemeral. **Deliberately not done yet, documented not hidden**: no automatic merge-back of a worker's worktree branch (Grok Build's own pattern is an explicit "apply" step, not automatic — a reasonable later model), no automatic cleanup of abandoned worktrees. `remove_worktree()` exists and is tested but nothing calls it automatically. The existing advisory file-touch conflict *detection* is untouched — this adds a structural layer on top, not a replacement. Fully live-tested against real `git` operations in tempdir repos (not mocked) — 9/9 passing, including two concurrent worktrees proven genuinely independent. Also confirmed stale: issue #1090 is already fixed upstream (commit `0a66fbcd2`), dropped as this item's justification; worktree isolation stands on conflict-resolution merits alone now. And: "one-level fan-out" is a root/deep-mode boolean gate not a depth counter (deep-swarm has no depth limit today, only a 1000-member cap), and the TUI's "worktree manager" role is aspirational dead comment text. |
