...
| 8 | **Orchestration-as-script** (`/workflow`-style templates) | Grok Build | **Shipped: first slice (templates, no scripting yet).** New `crates/jcode-app-core/src/workflow_template.rs` — `WorkflowTemplate`/`TemplateNode`/`WorkflowParameter`, `{{param}}` placeholder substitution into real `PlanItem`s, persisted at `~/.jcode/workflows/<name>.json` via the same atomic-write-with-recovery primitives `rewind_store.rs` already uses. **Scoping correction, source-verified**: `starlark = "0.14"` was already added to this crate in Phase 1 (for execpolicy) — this stays greenfield-free by not adding `rhai` at all; when real scripting logic is needed inside a template later, the plan is to reuse the existing Starlark dependency instead of introducing a second interpreter. `VersionedPlan` confirmed already durably persisted (item #2) — this slice extends that with the templating capability, doesn't rebuild persistence. 11/11 new tests, real-disk round-trip tested (not mocked). **Not yet done**: no agent tool wiring (types + persistence only, matching Mission Engine's own first-slice shape), no Starlark scripting inside a template. |
