...
| 2 | **Worktree-per-subagent swarm isolation** under jcode's existing coordinator | Codex + Grok Build (convergent) | **Revised after a real source read.** `VersionedPlan` (`crates/jcode-plan`) is already durably persisted (`server/swarm_persistence.rs`, atomic writes + versioning), not ephemeral. Workers today share the parent's working dir outright — zero worktree isolation exists, and no `git2`/`gix` dependency or `git worktree add` call anywhere (all git is shelled out). Reusable scaffolding found: `git_common_dir_for()`/`swarm_id_for_dir()` already detect worktrees and derive a shared swarm id from the common `.git` dir. **Dropped as justification: issue #1090 is already fixed** (commit `0a66fbcd2`, included in `v0.81.2`) — the daemon no longer kills live headless workers on idle. Worktree isolation is now justified purely on conflict-resolution grounds (still valid), not on #1090. Also note: "one-level fan-out" is a root/deep-mode boolean gate, not a depth counter — deep-swarm mode actually has no depth limit today (only a 1000-member cap + concurrency budget), and the TUI's "worktree manager" role is aspirational comment text with zero backing implementation. |
| 3 | **Guardian-style auto-approval reviewer** | Codex | **Revised after a real source read.** The `pre_tool` hook (`jcode-base/src/hooks.rs`) checks out exactly as expected — real plug-in point. But there is **no general interactive-session approval-prompt system to intercept** — the only one that exists (`SafetySystem`/`request_permission`, `jcode-base/src/safety.rs`) is restricted to ambient/autonomous sessions only, always returns non-blocking, and its `wait: true` "block until decided" flag is a confirmed no-op. Real approve/deny today only happens via a debug-socket handler, not a TUI dialog. **Open scope decision**: does Guardian only auto-answer ambient-session permission requests (narrower, matches what exists), or does it need a new general-session review path too (broader, more work)? Separately, jcode already has a more sophisticated deterministic command-risk classifier than assumed (`jcode-command-risk`: `Safe/Low/Confirm/Catastrophic` blast-radius classification with a "Reflect"-and-resubmit-with-justification mechanic, wired into `BashTool`) — **needs a decision: does execpolicy (#6) replace this, layer on top, or migrate its rule tables into Starlark?** |
...
| 5 | **Layered OS sandboxing** (bwrap/Landlock Linux, Seatbelt macOS, ACL/WFP Windows) | Codex | jcode has no OS-level sandboxing today beyond hook-based gating — confirmed zero scaffolding to duplicate (`grep` for sandbox-exec/seatbelt/bubblewrap/landlock/seccomp/bwrap: no hits). **Real architectural gap found, not just missing scope**: file-edit tools (`WriteTool` etc.) call `tokio::fs::write` directly in-process — no subprocess spawned at all. A sandbox built only by wrapping the bash tool's subprocess spawn point (`bash.rs::build_shell_command`, which itself has **two** separate spawn sites plus a third detached/reload path needing separate wrapping) would **silently miss all file writes/edits**. Must decide before starting: sandbox the whole process, or re-route file tools through a sandboxed helper. |
| 6 | **Execpolicy-as-Starlark** command classification | Codex | Testable, versionable policy files — but see item #3's open decision: jcode already has `jcode-command-risk`, a real deterministic blast-radius classifier wired into `BashTool` (not "ad-hoc allow/deny logic" as originally assumed) — building both side-by-side risks two overlapping gates on the same call. |
...
| 8 | **Orchestration-as-script** (`/workflow`-style, `rhai` crate) | Grok Build | **Revised.** `VersionedPlan` is already durably persisted (see item #2) — this extends that existing layer with a genuinely new capability (reusable, parameterized, replayable plan templates), not building persistence from scratch. `rhai`/`starlark`/any embedded-scripting dependency confirmed absent today (no hits in `Cargo.toml`/`Cargo.lock`, direct or transitive) — greenfield addition either way. |
...
- **Phase 4 — Memory**: two-phase consolidation (#9). Lowest urgency since jcode's memory system is already competitive as-is. Consider building Phase 3/4's scheduling need on Mission Engine's supervisor loop (§6 cross-cutting finding) rather than inventing separate schedulers.
