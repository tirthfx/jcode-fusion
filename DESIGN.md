...
| 7 | **ACP support** (close specific gaps + `jcode.dev/*` vendor extensions) | Grok Build | **Revised after a real source read: jcode's ACP adapter (`src/cli/acp.rs`, 2,188 lines) is NOT a stub** — it's a real, actively-shipped JSON-RPC server (`initialize`, `session/new`, `session/load`/`resume`, streaming `session/prompt`, `session/cancel`/`close`, extension mechanism), confirmed via changelog cross-check as actively developed. **Session-scoped MCP servers: shipped.** `session/new`'s `mcpServers` param now actually connects (`Request::McpConnectServer`, reuses the existing per-session `McpManager` + the `mcp` tool's own ad-hoc `connect` action, rather than the feared shared-bootstrap-path rewrite) — real gap closed, source-verified before and after. **Client-callback plumbing: shipped** (`AcpRuntime::send_client_request`/`route_response`) — the adapter can now send a request to the ACP host and correlate its response, closing the "handle_message treats every response as malformed" blocker. **Still open**: no real caller yet — `fs/read_text_file`, `session/request_permission`, `terminal/*` still handled server-side, since the tools that would delegate (`WriteTool`/`ReadTool`) run in the daemon process, a different process from this adapter, needing a new cross-process hop before they can reach it; auth negotiation over ACP; `initialize` ignoring the client's requested protocol version (checked, confirmed low-value — jcode only implements v1). |
