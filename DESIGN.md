...
| 7 | **ACP support** (close specific gaps + `jcode.dev/*` vendor extensions) | Grok Build | **Revised after a real source read: jcode's ACP adapter (`src/cli/acp.rs`, 2,188 lines) is NOT a stub** — it's a real, actively-shipped JSON-RPC server (`initialize`, `session/new`, `session/load`/`resume`, streaming `session/prompt`, `session/cancel`/`close`, extension mechanism), confirmed via changelog cross-check as actively developed. **Session-scoped MCP servers: shipped.** `session/new`'s `mcpServers` param now actually connects (`Request::McpConnectServer`, reuses the existing per-session `McpManager` + the `mcp` tool's own ad-hoc `connect` action, rather than the feared shared-bootstrap-path rewrite) — real gap closed, source-verified before and after. **Client-callback plumbing: shipped end-to-end, both hops.** Adapter-to-host (`AcpRuntime::send_client_request`/`route_response`) closes the "handle_message treats every response as malformed" blocker; daemon-to-adapter (`server/acp_callback.rs::send_acp_callback`, `ServerEvent::AcpCallbackRequest`/`Request::AcpCallbackResponse`) closes the cross-process blocker, reusing `SwarmMember`'s existing per-session `event_tx` to reach a live connection from anywhere in the daemon. A real high-severity bug (the adapter's relay originally blocked its own event-read loop for up to 120s per callback) was caught by review and fixed by spawning it, mirroring how `handle_session_prompt` already spawns `run_prompt`. **Still open, now the only piece**: `WriteTool`/`ReadTool` don't call any of this yet — they still do file I/O in-process, unconditionally, for every session type; wiring them in is the highest-blast-radius remaining change in the project. Auth negotiation over ACP checked and likely not a real gap (jcode's out-of-band login model). `initialize` ignoring the client's requested protocol version confirmed low-value. |
...
| 9 | **Two-phase memory consolidation** (leased jobs + single-locked consolidator) | Codex | Formalizes jcode's own documented "Ambient Garden" TODO — confirmed via real code read that this is genuinely unimplemented (only embedding-backfill is wired today, fire-and-forget after each ambient cycle). Builds on existing `MemoryManager`/`MemoryGraph` (`jcode-base/src/memory.rs`, `jcode-memory-types/src/graph.rs`, which already has `cascade_retrieve`). **Phase 1 (leased jobs) shipped in full**: the leasing/claiming primitive (`crates/jcode-app-core/src/memory_consolidation.rs`, retry backoff + crash-loop protection), real extraction (`extract_claimed_session`, wired to `MemoryManager::extract_from_transcript` — an already-existing, already-shipped Haiku-sidecar extractor that had zero callers anywhere before this, no new LLM pipeline built from scratch), and periodic ambient-runner wiring (opt-in, `JCODE_FUSION_MEMORY_CONSOLIDATION=1`, alongside the existing embedding-backfill step). A real stagnation bug (a naive directory-listing batch would permanently stall on the same fixed prefix once those sessions were extracted) was caught by review and fixed with a bounded random skip + small-install fallback. Documented, unfixed gaps: this trigger and the pre-existing interactive-CLI-exit extraction trigger don't share completion tracking (possible redundant extraction); coverage of a very large session store isn't uniform, just no longer permanently stuck. **Item #9 now complete.** Phase 2's render+locked-write primitive (`memory_consolidator.rs`) shipped, plus a periodic trigger (`run_memory_md_consolidation`, opt-in `JCODE_FUSION_MEMORY_MD=1`, wired alongside Phase 1's own ambient hook). No LLM sub-agent was built for this, deliberately: the consolidator is plain deterministic Rust with no tool-calling surface at all, so the design's "no-network, no-approval" requirement is satisfied by construction rather than needing an enforced sandbox around an agent that would have nothing to restrict. Six real bugs across both phase-2 slices fixed before shipping — the two most significant: the lock originally only covered the file write, not the memory-set read that fed it (restructured to gather inside the same locked section); the rendered document's embedded timestamp meant every cycle rewrote the file even with unchanged content, undermining the module's own "stable git diff" goal (fixed with a timestamp-stripped comparison before writing). |
