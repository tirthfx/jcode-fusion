...
| **3 — Ecosystem** | ACP support (#7), orchestration-as-script (#8) | **In progress** — orchestration-as-script first slice (workflow templates) shipped. ACP gaps not started. |
...
2. **Phase 3, ACP gaps (#7) — real scoping, not yet started**: of the four documented gaps, session-scoped MCP servers (`mcpServers` param is currently validated then silently discarded, see `validate_acp_mcp_servers` in `src/cli/acp.rs`) is the most concretely valuable, but source-verified to be a genuinely cross-cutting change — the ACP adapter is a *client* of the daemon over the same `Request::Subscribe` protocol every other client (TUI, headless) uses, so wiring session-scoped MCP servers through means extending `Request::Subscribe` itself and the shared new-session-creation path in `server/client_lifecycle.rs`, not something ACP-side alone can do. Real blast radius: every client type, not just ACP. Needs a dedicated slice with room for full-suite verification, not a rushed add-on. Client-side fs/permission/terminal callbacks (bidirectional RPC) is the biggest of the four gaps and should come after. Protocol-version negotiation (`initialize` always returns v1 regardless of what the client requested) was checked and found to have near-zero real value today — jcode only implements v1, ACP versions start at 1, so "always return 1" is already the correct response for any real client; **not worth a slice** unless a second protocol version is ever implemented. Auth-method negotiation via ACP itself is real but requires exposing jcode's existing multi-provider OAuth flows through a new `authenticate` RPC — scope similarly to session-scoped MCP, not attempted yet.
3. **Phase 4 (memory consolidation)** not started — per the resolved scheduler question above, model its periodic-loop piece on Ambient Mode's existing runner, not Mission Engine's supervisor.
...
6. **Lesson from the rewind slice, keep applying**: run the *full* `cargo test -p jcode-app-core --lib` after every slice that touches shared infrastructure.
7. Manual/live TUI verification (a real login, `JCODE_FUSION_SWARM_WORKTREES=1` with a real swarm spawn, a real `apply` call) still hasn't happened beyond example-based/unit-level demos — the single biggest unverified assumption in the whole project. The user ran a real live-credentialed smoke test as of 2026-08-30 (see `claude-code-build/SESSION_2_MEMORY.md`) — check there for the result before assuming this is still fully open.
8. Update this file at the end of every session — status table, session log entry, next steps — before ending.
