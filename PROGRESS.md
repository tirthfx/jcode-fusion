...
- `WorkflowTemplate { name, description, parameters: Vec<WorkflowParameter>, nodes: Vec<TemplateNode> }`, `WorkflowParameter { name, description, default: Option<String> }` (a `None` default means required), `TemplateNode` mirroring the subset of `PlanItem` a template actually needs (`id`, `content`, `priority`, `subsystem`, `file_scope`, `blocked_by` — deliberately *not* `status`/`assigned_to`, since every instantiation always starts fresh regardless of a prior run).
- `{{param_name}}` placeholder substitution across `content`/`subsystem`/`file_scope`. `instantiate()` refuses (doesn't silently proceed) on a missing required parameter, and separately refuses on any leftover `{{...}}`-shaped placeholder after substitution — a template referencing an undeclared parameter is treated as a template-authoring bug, not something to ship with a literal placeholder baked into a real task.
- `validate()` (called by both `save` and `instantiate`) rejects empty names, empty node lists, duplicate node ids, dangling `blocked_by` references, and duplicate parameter names — cheap structural checks up front rather than a confusing failure much later when a broken template hits real swarm-plan construction.
...
**Deliberately not done this slice** (documented, not silently skipped, matching the "smallest coherent first slice" shape Phase 0 started with): no agent tool wiring yet — there's no `workflow` tool action a live session can call to save/run a template. This slice is types + persistence + instantiation logic only. No Starlark scripting inside a template (see above). No template listing/discovery. No versioning/migration story if the template shape changes later.

11 new tests, all passing: parameter substitution with declared and default values, a missing-required-parameter refusal, an undeclared-parameter reference refusal, every `validate()` rejection case (no nodes, duplicate node id, dangling `blocked_by`, duplicate parameter name), a real disk round-trip (save → load, `tokio::test` against an isolated `JCODE_HOME`, matching every other module's real-not-mocked testing convention), a refused-invalid-template-never-written check, an unknown-name load failure, and the path-traversal-shaped-name sanitization check.

## Next steps (pick up here)

1. **Phase 3, next real slice**: wire `workflow_template` into an actual agent tool action (save/list/run) — this slice deliberately stopped at types+persistence, matching how Mission Engine's first slice also stopped before wiring a tool. `run` should take a template name + parameter values, call `instantiate()`, then feed the resulting `Vec<PlanItem>` into whatever already builds a `VersionedPlan` from plan items (`server/comm_plan.rs` — not yet read for this slice, read it first).
